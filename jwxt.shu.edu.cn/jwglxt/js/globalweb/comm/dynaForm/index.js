...
var oldForms;
(function($) {
    /** 不管模式1,2,3 都会把原先class为by_form(必须)的清空
     ...
     * finallyFun dom表单渲染好后执行的方法 比如：级联
      */ 
      ...
      $.dom = function(str) {
        ...
        var formStr = 
        `<div class="side-box">
            <form name="form" onsubmit="return false;" method="post" class="form-dynamic form-horizontal sl_all_form" role="form" id="subForm" name="subForm" data-toggle="validation">
        ...
        DynamicForm.prototype.bindFun = function(){
            ...
            $(document).off('click','#dyn_search_go').on('click','#dyn_search_go', function(event){
                if(!$(`${that.allOpt.id} #subForm`).valid()){
                     console.log('校验')   
                    return
                }
        ...
        DynamicForm.prototype.getDom = function(){
            ...
            // 复制再清空原来的表单
            $(".by_form select").chosen('destroy');//解绑 否则下拉失效
            that.byFormsDom = $('.by_form .sl_all_form .row').clone(true);
            that.byBtnExtraDom = $('.by_form .sl_aff_btn .extra').clone(true);//查询左侧dom
            $('.by_form').remove();
            var extraBox = that.formDom.querySelector('.extra-box');

            $(extraBox).empty().append(that.byBtnExtraDom);
            ...
            // console.log(newCzList, allNameList,'allNameList=')
            if(that.list && that.list.length){
                ...
                if(that.allOpt.mode =='3'){
                    console.log(allNameList,'333333****')
                    ...
                    allNameList.forEach(function(item,index){
                        ...
                        if(dt.length){
                            ...
                            var comStr = `
                            <div class="${index}-it col-sm-6 col-md-${that.allOpt.colSmNum}${that.allOpt.maxShowNum && index+1>that.allOpt.maxShowNum?' hiddenBox':''}">${st.outerHTML}</div>
                            `
                    ...
                    console.log(allList,'allList==2')
                    $(".side-box .sl_all_form select").trigger("chosen");
                }
            }else{
                ...
                groups.each(function(index,item){
                    var comStr = `
                    <div class="${index}-it col-sm-6 col-md-${that.allOpt.colSmNum}${that.allOpt.maxShowNum && index+1>that.allOpt.maxShowNum?' hiddenBox':''}"></div>
                    `
                    $('.side-box .sl_all_form .row-content').append(comStr);
